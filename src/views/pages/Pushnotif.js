...
=======
  ...
  useEffect(() => {
    async function getData() {
      await axios.get('http://localhost/banners/all').then(res => {
        setData(res.data);
        console.log(res.data);
        setDataSize(res.data.length);
        setLoadingData(false);
  ...
  async function submitForm() {
    ...
    axios
      .all([
        axios.post('http://localhost/photos/upload', formData),
        axios.post('http://localhost/banners/create', {
>>>>>>> master
          code: code,
          name: name,
          details: details,
  ...
  return (
    <div>
      ...
      <Modal isOpen={pics} toggle={picsClose} style={{ width: 800, height: 500 }}>
        <ModalHeader style={{ backgroundColor: '#1d1594', color: '#ffffff' }}>Imagen Banner</ModalHeader>
        <ModalBody style={{ alignSelf: 'center' }}>
          <img style={{ padding: 10 }} src={'http://18.217.43.19/uploads/banners/' + urlPic} />
        </ModalBody>
        <ModalFooter>
          <Button color="primary">Cambiar Imagen</Button>
